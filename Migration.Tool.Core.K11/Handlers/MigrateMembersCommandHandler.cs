    ...
    : IRequestHandler<MigrateMembersCommand, CommandResult>, IDisposable
{
    ...
    public async Task<CommandResult> Handle(MigrateMembersCommand request, CancellationToken cancellationToken)
    {
        ...
        foreach (var k11User in k11CmsUsers)
        {
            ...
            var xbkMemberInfo = MemberInfo.Provider.Get(k11User.UserGuid);
            ...
            if (k11User.CmsUserSettingUserSettingsUserNavigation is null)
            {
                logger.LogError("User settings for user {UserName} with UserGuid {UserGuid} not found", k11User.UserName, k11User.UserGuid);
                continue;
            }
