...
public class MediaFileInfoMapper : EntityMapperBase<MediaFileInfoMapperSource, MediaFileInfo>
{
    ...
    protected override MediaFileInfo MapInternal(MediaFileInfoMapperSource args, MediaFileInfo target, bool newInstance, MappingHelper mappingHelper, AddFailure addFailure)
    {
        ...
        var targetCreatedMemberId = _keyMappingContext.MapSourceKey<Toolkit.KX13.Models.CmsUser, KXP.Models.CmsMember, int?>(
            s => s.UserId,
            s => s.UserGuid,
        ...
        else if (mappingHelper.TranslateIdAllowNulls<Toolkit.KX13.Models.CmsUser>(c => c.UserId, mediaFile.FileCreatedByUserId, out var createdByUserId))
        {
            target.SetValue(nameof(target.FileCreatedByUserID), createdByUserId);
        }

        var targetModifiedMemberId = _keyMappingContext.MapSourceKey<Toolkit.KX13.Models.CmsUser, KXP.Models.CmsMember, int?>(
            s => s.UserId,
            s => s.UserGuid,
        ...
        else if (mappingHelper.TranslateIdAllowNulls<Toolkit.KX13.Models.CmsUser>(c => c.UserId, mediaFile.FileModifiedByUserId, out var modifiedByUserId))
        {
