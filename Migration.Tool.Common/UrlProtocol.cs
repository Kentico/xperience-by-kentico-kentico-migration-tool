...
public class UrlProtocol : IDisposable, IAsyncDisposable
{
    ...
    public UrlProtocol(ToolConfiguration toolConfiguration)
    {
        string? dirName = Path.GetDirectoryName(toolConfiguration.UrlProtocol);
        string? fileName = Path.GetFileNameWithoutExtension(toolConfiguration.UrlProtocol);
        string? extension = Path.GetExtension(toolConfiguration.UrlProtocol);
        ...
        migrationToAssets = !toolConfiguration.MigrateMediaToMediaLibrary;
    }
