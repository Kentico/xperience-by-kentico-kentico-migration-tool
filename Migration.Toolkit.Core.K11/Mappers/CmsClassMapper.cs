...
public class CmsClassMapper(ILogger<CmsClassMapper> logger,
    ...
    : EntityMapperBase<CmsClass, DataClassInfo>(logger,
    primaryKeyMappingContext, protocol)
{
    private readonly KeyMappingContext _keyMappingContext = keyMappingContext;

    protected override DataClassInfo? CreateNewInstance(CmsClass source, MappingHelper mappingHelper, AddFailure addFailure) =>
        DataClassInfo.New();

    protected override DataClassInfo MapInternal(CmsClass source, DataClassInfo target, bool newInstance, MappingHelper mappingHelper,
        AddFailure addFailure)
    {
        ...
        if (source.ClassResource?.ResourceName is { } resourceName)
        {
            ...
            logger.LogDebug("{ClassName} is {@Properties}", source.ClassName, new { isCustomizableSystemClass, classIsCustom, source.ClassResourceId, source.ClassResource?.ResourceName });
        }
        ...
        if (mappingHelper.TranslateIdAllowNulls<CmsResource>(c => c.ResourceId, source.ClassResourceId, out var resourceId))
        {
