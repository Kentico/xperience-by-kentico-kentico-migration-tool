...
using Migration.Toolkit.Core.K11.Auxiliary;
...
public class ContentItemMapper(ILogger<ContentItemMapper> logger,
    ...
    : UmtMapperBase<CmsTreeMapperSource>
{
    ...
    private readonly KxpPageFacade _pageFacade = pageFacade;
    ...
    protected override IEnumerable<IUmtModel> MapInternal(CmsTreeMapperSource source)
    {
        ...
        foreach (var cmsDocument in migratedDocuments)
        {
            ...
            var commonDataModel = new ContentItemCommonDataModel
            {
                ...
                //     contentItemCommonDataPageBuilderWidgets, // PatchPageBuilderWidgets(cmsDocument.DocumentPageBuilderWidgets, cmsTree.NodeID, "cms.node", out _),
                // ContentItemCommonDataPageTemplateConfiguration =
                //     contentItemCommonDataPageTemplateConfiguration, // PatchPageTemplateConfiguration(cmsDocument.DocumentPageTemplateConfiguration, cmsTree.NodeID, "cms.node", out var _)
            ...
            if (cmsTree.NodeClass.ClassIsCoupledClass)
            {
                ...
                var coupledDataRow = coupledDataService.GetSourceCoupledDataRow(cmsTree.NodeClass.ClassTableName, primaryKeyName, cmsDocument.DocumentForeignKeyValue);
            ...
            if (keyMappingContext.GetGuid<CmsUser>(u => u.UserId, u => u.UserGuid, cmsDocument.DocumentCreatedByUserId) is (true, var createdByUserGuid))
            {
            ...
            if (keyMappingContext.GetGuid<CmsUser>(u => u.UserId, u => u.UserGuid, cmsDocument.DocumentModifiedByUserId) is (true, var modifiedByUserGuid))
            {
    ...
    private IEnumerable<IUmtModel> MigrateDraft(CmsVersionHistory checkoutVersion, CmsTree cmsTree, string sourceFormClassDefinition, string targetFormDefinition, Guid contentItemGuid,
        ...
        foreach (var (key, value) in properties)
        {
            ...
            if (editingFcm != null)
            {
                        ...
                        .SingleOrDefault(x => x.OldFormComponent == editingFcm.FormComponentIdentifier) is var (oldFormComponent, newFormComponent))
                {
                    ...
                    {
                        case Kx12FormComponents.Kentico_PathSelector:
                        {
    ...
    )
    {
        ...
        //if (cmsTree.NodeClass.ClassName.Equals("Panos.TileItem")) Debugger.Break();
        ...
        foreach (var columnName in newColumnNames)
        {
            ...
            var convertToAsset = fieldMigration?.Actions?.Contains(TcaDirective.ConvertToAsset) ?? false;
            if (convertToAsset)
            {
                ...
                {
                    // List<AssetRelatedItem> fallback = target.TryGetValue(columnName, out var identifier) && identifier is Guid i
                    //     ? new List<AssetRelatedItem> { new() { Identifier = i } }
                    //     : [];
                    List<AssetRelatedItem> fallback = [];
                    target.SetValueAsJson(columnName, fallback);
                }
