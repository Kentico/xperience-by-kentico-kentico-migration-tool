...
public class CountryMigrator
{
    ...
    public void MigrateCountriesAndStates()
    {
        ...
        var kx13Countries = kx12Context.CmsCountries.AsNoTracking();
        foreach (var kx13CmsCountry in kx13Countries)
        {
            ...
            if (mapped is (var countryInfo, var newInstance) { Success: true })
            {
                try
                {
                    ...
                    _primaryKeyMappingContext.SetMapping<KX12M.CmsCountry>(r => r.CountryId, kx13CmsCountry.CountryId, countryInfo.CountryID);
                }
        ...
        var kx13States = kx12Context.CmsStates.AsNoTracking();
        foreach (var kx13CmsState in kx13States)
        {
            ...
            if (mapped is (var stateInfo, var newInstance) { Success: true })
            {
                try
                {
                    ...
                    _primaryKeyMappingContext.SetMapping<KX12M.CmsState>(r => r.StateId, kx13CmsState.StateId, stateInfo.StateID);
                }
                catch (Exception exception)
                {
