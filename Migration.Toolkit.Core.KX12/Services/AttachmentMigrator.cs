...
public class AttachmentMigrator
{
    ...
    {
        using var kx12Context = _kx12ContextFactory.CreateDbContext();
        var attachment = kx12Context.CmsAttachments.SingleOrDefault(a => a.AttachmentGuid == kx13CmsAttachmentGuid);
        if (attachment == null)
        {
    ...
    public MigrateAttachmentResult MigrateAttachment(KX12M.CmsAttachment kx13CmsAttachment, string? additionalMediaPath = null)
    {
        ...
        using var kx12Context = _kx12ContextFactory.CreateDbContext();
        var site = kx12Context.CmsSites.FirstOrDefault(s=>s.SiteId == kx13CmsAttachment.AttachmentSiteId) ?? throw new InvalidOperationException("Site not exists!");
    ...
    private KX12M.CmsDocument? GetKx13CmsDocument(int documentId)
    {
