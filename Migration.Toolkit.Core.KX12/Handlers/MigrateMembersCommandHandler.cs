...
public class MigrateMembersCommandHandler : IRequestHandler<MigrateMembersCommand, CommandResult>, IDisposable
{
    ...
    {
        // var migratedSiteIds = _toolkitConfiguration.RequireExplicitMapping<KX12M.CmsSite>(s => s.SiteId).Keys.ToList();
        ...
        var kx13CmsUsers = kx12Context.CmsUsers
        ...
        foreach (var kx13User in kx13CmsUsers)
        {
            ...
            //     _primaryKeyMappingContext.SetMapping<KX12M.CmsUser>(r => r.UserId, kx13User.UserId, xbkMemberInfo.MemberID);
            //     continue;
            // }
            ...
            if (xbkMemberInfo?.MemberName == USER_PUBLIC || kx13User.UserName == USER_PUBLIC)
            {
                ...
                // {
                //     _primaryKeyMappingContext.SetMapping<KX12M.CmsUser>(r => r.UserId, kx13User.UserId, xbkMemberInfo.UserID);
                // }
    ...
    // {
    //     await using var kx13DbContext = await _kx12ContextFactory.CreateDbContextAsync(cancellationToken);
    //
    ...
    //
    //         var xbkSiteId = _primaryKeyMappingContext.RequireMapFromSource<KX12M.CmsSite>(u => u.SiteId, kx13UserSite.SiteId);
    //
    ...
    private async Task<bool> SaveUserUsingKenticoApi(CancellationToken cancellationToken, IModelMappingResult<MemberInfo> mapped, KX12M.CmsUser kx13User)
    {
        if (mapped is { Success : true } result)
        {
            ...
            _primaryKeyMappingContext.SetMapping<KX12M.CmsUser>(r => r.UserId, kx13User.UserId, memberInfo.MemberID);
            return true;
        }
