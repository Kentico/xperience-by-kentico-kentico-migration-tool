...
public class MigrateSitesCommandHandler : IRequestHandler<MigrateSitesCommand, CommandResult>
{
    ...
    public async Task<CommandResult> Handle(MigrateSitesCommand request, CancellationToken cancellationToken)
    {
        ...
        foreach (var kx13CmsSite in kx12Context.CmsSites.Include(s => s.Cultures))
        {
            ...
            {
                await using var ctx = await _kx12ContextFactory.CreateDbContextAsync(cancellationToken);
            ...
            //var homePagePath = KenticoHelper.GetSettingsKey(_kx12ContextFactory, kx13CmsSite.SiteId, SettingsKeys.CMSHomePagePath);
            var cookieLevel = KenticoHelper.GetSettingsKey(_kx12ContextFactory, kx13CmsSite.SiteId, SettingsKeys.CMSDefaultCookieLevel) switch
            {
            ...
            var storeFormerUrls = KenticoHelper.GetSettingsKey(_kx12ContextFactory, kx13CmsSite.SiteId, "CMSStoreFormerUrls") is string storeFormerUrlsStr
            ...
            var webSiteChannelResult = await _importer.ImportAsync(new WebsiteChannelModel
            {
                ...
                // WebsiteChannelHomePage = homePagePath, // TODO tomas.krch: 2023-12-14 KX12
            ...
            {
                var cmsReCaptchaPublicKey = KenticoHelper.GetSettingsKey(_kx12ContextFactory, kx13CmsSite.SiteId, "CMSReCaptchaPublicKey") as string;
                var cmsReCaptchaPrivateKey = KenticoHelper.GetSettingsKey(_kx12ContextFactory, kx13CmsSite.SiteId, "CMSReCaptchaPrivateKey") as string;

                WebsiteCaptchaSettingsInfo? reCaptchaSettings = null;
                var cmsReCaptchaV3PrivateKey = KenticoHelper.GetSettingsKey(_kx12ContextFactory, kx13CmsSite.SiteId, "CMSReCaptchaV3PrivateKey") as string;
                var cmsRecaptchaV3PublicKey = KenticoHelper.GetSettingsKey(_kx12ContextFactory, kx13CmsSite.SiteId, "CMSRecaptchaV3PublicKey") as string;
                var cmsRecaptchaV3Threshold = KenticoHelper.GetSettingsKey<double>(_kx12ContextFactory, kx13CmsSite.SiteId, "CMSRecaptchaV3Threshold");
    ...
    private string GetSiteCulture(CmsSite site)
    {
        ...
        var siteCulture = site.SiteDefaultVisitorCulture
                          ?? KenticoHelper.GetSettingsKey(_kx12ContextFactory, site.SiteId, SettingsKeys.CMSDefaultCultureCode);
