...
public class MigrateCustomModulesCommandHandler : IRequestHandler<MigrateCustomModulesCommand, CommandResult>
{
    ...
    private async Task MigrateClasses(EntityConfiguration entityConfiguration, CancellationToken cancellationToken)
    {
        ...
        var kx13ClassesResult = kx12Context.CmsClasses
        ...
        while (kx13Classes.GetNext(out var di))
        {
            ...
            if (kx13Class.ClassInheritsFromClassId is { } classInheritsFromClassId && !_primaryKeyMappingContext.HasMapping<KX12M.CmsClass>(c => c.ClassId, classInheritsFromClassId))
            {
            ...
            if (kx13ResourceName != null && Kx13SystemResource.All.Contains(kx13ResourceName) && !Kx13SystemResource.ConvertToNonSysResource.Contains(kx13ResourceName))
            {
                if (!Kx13SystemClass.Customizable.Contains(kx13Class.ClassName))
                {
                    ...
                    //     .NotSupportedSkip<KX12M.CmsClass>()
    ...
    private async Task MigrateMemberClass(CancellationToken cancellationToken)
    {
        ...
        var cmsUser = kx12Context.CmsClasses.FirstOrDefault(x => x.ClassName == Kx13SystemClass.cms_user);
        var cmsUserSettings = kx12Context.CmsClasses.FirstOrDefault(x => x.ClassName == Kx13SystemClass.cms_usersettings);

        if (cmsUser == null)
        {
            _protocol.Warning<KX12M.CmsUser>(HandbookReferences.InvalidSourceData<KX12M.CmsUser>().WithMessage($"{Kx13SystemClass.cms_user} class not found"), null);
            return;
        }

        if (cmsUserSettings == null)
        {
            _protocol.Warning<KX12M.CmsUserSetting>(HandbookReferences.InvalidSourceData<KX12M.CmsUserSetting>().WithMessage($"{Kx13SystemClass.cms_usersettings} class not found"), null);
            return;
        }
    ...
    private async Task MigrateAlternativeForms(CmsClass kx13Class, DataClassInfo xbkDataClass, CancellationToken cancellationToken)
    {
        ...
        var kx13AlternativeForms = kx12Context.CmsAlternativeForms
    ...
    private async Task<List<KX12M.CmsClass>> GetResourceClasses(int kx13ResourceId, CancellationToken cancellationToken)
    {
        await using var kx12Context = await _kx12ContextFactory.CreateDbContextAsync(cancellationToken);

        return await kx12Context.CmsClasses.Where(x => x.ClassResourceId == kx13ResourceId).ToListAsync();
    }
    ...
    private async Task MigrateResources(CancellationToken cancellationToken)
    {
        ...
        var kx13CmsResources = kx12Context.CmsResources
            .Include(cr => cr.Sites);
    ...
    // OBSOLETE
    // private void MigrateResourceToSiteMapping(List<int> migratedSites, ResourceInfo resourceInfo, KX12M.CmsResource kx13CmsResource)
    // {
    //     foreach (var migratedSiteId in migratedSites)
    //     {
    //         var xbkSiteId = _primaryKeyMappingContext.RequireMapFromSource<KX12M.CmsSite>(s => s.SiteId, migratedSiteId);
    //         try
    //         {
    ...
    private DataClassInfo? SaveClassUsingKxoApi(KX12M.CmsClass kx13Class, DataClassInfo kxoDataClass)
    {
    ...
    // private void MigrateClassSiteMappings(int dcId, int? dataClassId, KX12M.CmsClass kx13Class)
    // {
