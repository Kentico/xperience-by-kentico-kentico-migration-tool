...
public class MigrateUsersCommandHandler : IRequestHandler<MigrateUsersCommand, CommandResult>, IDisposable
{
    ...
    public async Task<CommandResult> Handle(MigrateUsersCommand request, CancellationToken cancellationToken)
    {
        ...
        foreach (var kx13User in kx13CmsUsers)
        {
            ...
            if (kx13User.UserPrivilegeLevel == (int)UserPrivilegeLevelEnum.GlobalAdmin && xbkUserInfo != null)
            {
                ...
                _primaryKeyMappingContext.SetMapping<KX12M.CmsUser>(r => r.UserId, kx13User.UserId, xbkUserInfo.UserID);
                continue;
            }
    ...
    private async Task<bool> SaveUserUsingKenticoApi(CancellationToken cancellationToken, IModelMappingResult<UserInfo> mapped, KX12M.CmsUser kx13User)
    {
        if (mapped is { Success : true } result)
        {
            ...
            _primaryKeyMappingContext.SetMapping<KX12M.CmsUser>(r => r.UserId, kx13User.UserId, userInfo.UserID);
            return true;
        }
    ...
    {
        var kx13CmsRoles = KX12Context.CmsRoles
            .Where(r =>
    ...
    private async Task MigrateUserRole(int kx13RoleId)
    {
        ...
        var kx13UserRoles = KX12Context.CmsUserRoles
            .Where(ur =>
        ...
        await foreach (var kx13UserRole in kx13UserRoles)
        {
            ...
            if (!_primaryKeyMappingContext.TryRequireMapFromSource<KX12M.CmsRole>(u => u.RoleId, kx13RoleId, out var xbkRoleId))
            {
            ...
            if (!_primaryKeyMappingContext.TryRequireMapFromSource<KX12M.CmsUser>(u => u.UserId, kx13UserRole.UserId, out var xbkUserId))
            {
                continue;
            }
