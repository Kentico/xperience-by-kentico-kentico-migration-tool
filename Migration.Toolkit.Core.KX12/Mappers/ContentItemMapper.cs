...
using Migration.Toolkit.Core.KX12.Auxiliary;
...
using Migration.Toolkit.KX12.Auxiliary;
...
public class ContentItemMapper : UmtMapperBase<CmsTreeMapperSource>
{
    ...
    protected override IEnumerable<IUmtModel> MapInternal(CmsTreeMapperSource source)
    {
        ...
        foreach (var cmsDocument in migratedDocuments)
        {
            ...
            if (checkoutVersion is { PublishFrom: null } draftVersion)
            {
                ...
                {
                    _logger.LogWarning("Failed to migrate checkout version of document with DocumentID={CmsDocumentDocumentId} VersionHistoryID={CmsDocumentDocumentCheckedOutVersionHistoryId}",
            ...
            if (_keyMappingContext.GetGuid<KX12M.CmsUser>(u => u.UserId, u => u.UserGuid, cmsDocument.DocumentCreatedByUserId) is (true, var createdByUserGuid))
            {
            ...
            if (_keyMappingContext.GetGuid<KX12M.CmsUser>(u => u.UserId, u => u.UserGuid, cmsDocument.DocumentModifiedByUserId) is (true, var modifiedByUserGuid))
            {
    ...
    private IEnumerable<IUmtModel> MigrateDraft(KX12M.CmsVersionHistory checkoutVersion, KX12M.CmsTree cmsTree, string sourceFormClassDefinition, string targetFormDefinition, Guid contentItemGuid,
