...
public class AttachmentsService : ActionResultServiceBase
{
    ...
    protected override CMSActionResult GetActionResultInternal()
    {
        ...
        if (pathAndFileName != null)
        {
            ...
            var mf = mediaFiles
                .FirstOrDefault(mf =>
                    (string)mf.FileCustomData.GetValue("__LegacyOriginalPath") == $"/{dir}" &&
                    mf.FilePath.EqualsCSafe(pathAndFileName)
                );
            
            if (mf != null && siteService.CurrentSite is { } site && pathAndFileName.LastIndexOf('/') is var fileNameIndex and >= 0)
            {
                var fileName = pathAndFileName[(fileNameIndex + 1)..];
                var mediaPath = MediaFileInfoProvider.GetMediaFilePath(mf, site.SiteName, SystemContext.WebApplicationPhysicalPath);
                var result = new CMSPhysicalFileResult(mediaPath)
                {
                    ContentType = mf.FileMimeType,
