...
> **New to migration development?** Start with the [developer training](https://docs.kentico.com/paths/xbyk-upgrade-developer) in our Learn portal.
...
| `--pages`                   | Enables migration of [pages](https://docs.kentico.com/x/AQqRBg) and their attachments.<br /><br />Pages can be migrated to either [website channel pages](https://docs.kentico.com/x/JwKQC) (default behavior) or [reusable content items](https://docs.kentico.com/x/content_items_xp) in Content hub.<br /><br />When migrating to website channel pages, ensure the target instance does not contain pages other than those created by previous runs of the Kentico Migration Tool.<br /><br />See: [Migration details for specific object types - Pages](#pages) | `--sites`, `--users`, `--page-types`                                 |
...
| `--contact-management`      | Enables migration of [contacts](https://docs.xperience.io/x/nYPWCQ), [activities](https://docs.xperience.io/x/oYPWCQ), countries, and states. Countries and states are migrated as part of contact management. The target instance must not contain any contacts or activities. May run for a long time depending on the number of contacts in the source database.                                                                                                                                                                                                  | `--users`, `--custom-modules`                                        |
...
- Countries and states are migrated as part of contact management.
...
"CommerceConfiguration": {
  "CommerceSiteNames": ["DancingGoatCore", "DancingGoatMvc"]
}
```
...
- A `SiteOriginName` custom field is created for customers. For registered users, this field's value is `null`.
...
| KxCmsDirPath                                                      | The absolute file system path of the **CMS** folder in the source Kentico Xperience 13, Kentico 12, or Kentico 11 administration project. Required to migrate media library files.<br /><br />**Note:** Media files must be present in the local file system; remote storage (Azure Blob, Amazon S3, etc.) is not supported as a source.                                                                                                                                                                                                                                                                                                              |
...
| AssetRootFolders                                                  | Dictionary defining the root folder for Asset content items per site: Key is site name (CMS*Site.SiteName). Value is in format */FolderDisplayName1/FolderDisplayName2/...\_                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| EntityConfigurations._&lt;object table name&gt;_.ExcludeCodeNames | Excludes objects with the specified code names from the migration. Particularly useful in multisite scenarios when migrating one site at a time—you can exclude entire sites by code name (using `CMS_Site`) or specific objects like media libraries or content types belonging to other sites.                                                                                                                                                                                                                                                                                                                                                      |
...
```json
{
  ...
  "Settings": {
    ...
    "CommerceConfiguration": {
      ...
      "OrderStatuses": {
        "XbKStatusCodeNameOne": ["KX13StatusCodeNameOne"],
        "XbKStatusCodeNameTwo": [
          "KX13StatusCodeNameTwo",
          "AnotherKX13StatusCodeName"
        ]
      },
      "KX13OrderFilter": {
        "OrderFromDate": "2024-01-01T00:00:00Z",
